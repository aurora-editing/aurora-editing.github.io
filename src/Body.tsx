...
import aurorafigure from './assets/aurorafigure.png'
    ...
    <main className='px-5 py-5 lg:px-40 lg:py-10'>
      <Section title="Abstract" hasTitle={false}>
        <p className="leading-7 [&:not(:first-child)]:mt-6 text-justify">
            ...
            <br />
            To address this, we meticulously curate the AURORA Dataset (Action-Reasoning-Object-Attribute), a collection of high-quality training data, human-annotated and curated from videos and simulation engines. We focus on a key aspect of quality training data: triplets (source image, prompt, target image) that contain a single meaningful visual change described by the prompt, ensuring truly minimal changes between source and target images.
            <br />
            To demonstrate the value of our dataset, we evaluate an AURORA-finetuned model on a new expert-curated benchmark (AURORA-Bench) covering eight diverse editing tasks. Our model significantly outperforms previous editing models as judged by human raters. For automatic evaluations, we find important flaws in previous metrics and caution their use for semantically hard editing tasks. Instead, we propose a new automatic metric that focuses on discriminative understanding.
            <br />
      ...
      <Section title="AURORA Dataset">
        ...
        <img src={aurorafigure} alt="Figure 2" style={{height: "auto", width: "auto"}}/>
      </Section>
      <Section title="AURORA-BENCH">
        <p className="leading-7 [&:not(:first-child)]:mt-6 text-justify">
          To holistically assess the editing abilities defined in our paper (object/attribute, global, action, reasoning, excluding viewpoint), we manually create a set of 400 image-edit-instruction pairs from 8 sources: AURORA-Bench. We ensure that AURORA-Bench allows studying out-of-distribution (OOD) transfer when a model is trained on AURORA Dataset, e.g., Sim2Real transfer from Kubric-Edit to real-world (spatial) reasoning or action edits outside of Action-Genome-Edit or Something-Something-Edit. Each of the 8 tasks contains 50 examples of image-prompt pairs that were either directly written by the authors or manually inspected for quality.
        </p>
      </Section>
      <Section title="Demo">
        <iframe
